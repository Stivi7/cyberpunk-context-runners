...
- Templates are copied as-is; Codex skills are generated from the skill templates

## Structure

- `agents/` - Agent definition files with roles, inputs, and outputs (copied into tool directories)
...
- `skills/` - Specialized context snippets for specific technical tasks (copied into tool directories)
- `tasks/` - Task definition templates
- `.cursor/rules/cyberpunk.mdc` - Cursor IDE rules (copied verbatim)
- `.codex/config.toml` - Codex project configuration (copied verbatim)
...
These templates are used by the `cyberpunk init` command to scaffold new projects with the standard cyberpunk agent and skills structure.
